...
export default function ReceivePage() {
  ...
  const profileLink = `masanawa.app/@${DEMO_USER.username}`
    ...
    <div className="flex max-w-lg flex-col gap-8">
      ...
      {tab === 'username' ? (
        <section className="flex flex-col items-center gap-6 rounded-2xl border border-border bg-card p-6 text-center">
          ...
          <div>
            <p className="font-mono text-lg font-semibold text-primary">@{DEMO_USER.username}</p>
            <p className="mt-1 text-sm text-muted-foreground">
              Anyone on Masanawa can send you naira or crypto with this username. No fees.
            </p>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row">
            <Button onClick={() => copy(`@${DEMO_USER.username}`, `@${DEMO_USER.username}`)}>
