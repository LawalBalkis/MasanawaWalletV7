...
import { tradeAction } from '@/lib/wallet/actions'
import {
  formatAsset,
  formatNgn,
  tradeFeeNgn,
...
export function TradeFlow({ holdings }: { holdings: AssetHolding[] }) {
  ...
  const insufficient =
    mode === 'buy' ? ngn + fee > ngnBalance : assetAmount > asset.balance
  const valid = ngn >= 1000 && !insufficient
      ...
      >
        ...
        <div className="flex flex-col gap-2">
          ...
          <p className="text-xs text-muted-foreground">
            Minimum {formatNgn(1000)} ·{' '}
            {mode === 'buy'
              ? `Naira balance ${formatNgn(ngnBalance)}`
              : `You hold ${formatAsset(asset.balance, asset)}`}
          </p>
        </div>
